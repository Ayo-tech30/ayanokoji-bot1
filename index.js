...
if (!TELEGRAM_TOKEN || !ADMIN_TELEGRAM_ID) {
    console.error('❌ ERROR: Missing environment variables!');
    console.error('Please set TELEGRAM_TOKEN and ADMIN_TELEGRAM_ID in Railway');
    process.exit(1);
}
...
║ ■ .close`;
        ...
        // ADMIN COMMANDS (Group only)
            ...
            if (action === 'on') {
                ...
                group.antilink let users = loadDB('users.json');
let groups = loadDB('groups.json');
let cards = loadDB('cards.json');
let botStats = {
    startTime: Date.now(),
